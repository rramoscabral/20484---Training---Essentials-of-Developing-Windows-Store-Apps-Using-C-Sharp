...
namespace ILoveNotes
{
    ...
    public sealed partial class ItemDetailPage : ILoveNotes.Common.NotesBasePage
    {
        ...
        void DrawDefaultAppBar()
        {
            ...
            {
                GetButtonFromBar("btnAddToDo").Visibility = Windows.UI.Xaml.Visibility.Visible;
            }
        }
        ...
        {
            MenuFlyout deleteMenu = new MenuFlyout();
            MenuFlyoutItem command = null;

            if (Note.Type == NoteTypes.ToDo && todoControl.HasSelectedItems)
            {
                command = new MenuFlyoutItem() { Text = "Delete ToDo" };
                command.Click += (s, args) => { DeleteTag(sender); };
                deleteMenu.Items.Add(command);
            }
            if (imgList.SelectedItems.Count > 0)
            {
                command = new MenuFlyoutItem() { Text = "Delete Image" };
                command.Click += (s, args) => { DeleteImageAsync(sender); };
                deleteMenu.Items.Add(command);
            }

            if (deleteMenu.Items.Count > 0)
            {
                MenuFlyoutSeparator separator = new MenuFlyoutSeparator();
                deleteMenu.Items.Add(separator);

                command = new MenuFlyoutItem() { Text = "Delete Note" };
                command.Click += (s, args) => { DeleteAsync(sender); };
                deleteMenu.Items.Add(command);

                deleteMenu.ShowAt((FrameworkElement)sender);
            }
            else
                DeleteAsync(sender);
        ...
        /// <summary>
        /// Change Pin Icon based on Note SecondaryTile status
        /// </summary>
        void TogglePinToStartButtonIcon()
        {
            ...
            if (TileManager.PinExists(Note.UniqueId))
            {
        ...
        private void btnOpenTagsPopupClick(object sender, RoutedEventArgs e)
        {
            ...
            var items = BuildTagList();

            listSuggestions.Items.Clear();
            foreach (var item in items)
            {
                listSuggestions.Items.Add(new ListViewItem() { Content = item.Key, IsSelected = item.Value });
            }
