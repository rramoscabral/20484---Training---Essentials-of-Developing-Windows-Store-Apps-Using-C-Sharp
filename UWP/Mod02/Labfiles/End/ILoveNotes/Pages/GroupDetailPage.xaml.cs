...
namespace ILoveNotes
{
    ...
    public sealed partial class GroupDetailPage : ILoveNotes.Common.NotesBasePage
    {
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        async private void btnSort_Click(object sender, RoutedEventArgs e)
        {
            PopupMenu menu = new PopupMenu();
            menu.Commands.Add(new UICommand("Created Date", Sort, 1));
            menu.Commands.Add(new UICommand("Tag", Sort, 2));
            menu.Commands.Add(new UICommand("Normal", Sort, 3));
        ...
        void Sort(object command)
        {
            var id = ((UICommand)command).Id.ToString();
            var sortSections = new ObservableCollection<Section>();
            switch (id)
            {
                case "1":
                    ...
                    sortSections.Add(today);
                    sortSections.Add(yesterday);
                    sortSections.Add(older);

                    this.DefaultViewModel["Sections"] = sortSections;
                    break;
                case "2":
                    var tags = notebook.Items.SelectMany(item => item.Tags).Distinct();
                    if (tags.Count() == 0)
                    {
                        Helpers.ShowMessageAsync("No tags were found, choose another filter", "Sort Notebook"); ;
                        return;
                    }
