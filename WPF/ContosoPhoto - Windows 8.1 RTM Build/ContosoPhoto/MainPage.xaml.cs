...
namespace ContosoPhoto
{
    public sealed partial class MainPage : Page
    {
        ...
        private bool CanFixRedEye = false;
        ...
        private void Photo_ManipulationDelta(object sender, ManipulationDeltaRoutedEventArgs e)
        {
            ...
            // Scale the photo (constrain the scale factor)
            double scaleX = PhotoTransform.ScaleX * e.Delta.Scale;
            scaleX = Math.Min(scaleX, 4.0);
            scaleX = Math.Max(scaleX, 1.0);
            PhotoTransform.ScaleX = scaleX;

            double scaleY = PhotoTransform.ScaleY * e.Delta.Scale;
            scaleY = Math.Min(scaleY, 4.0);
            scaleY = Math.Max(scaleY, 1.0);
            PhotoTransform.ScaleY = scaleY;
        }
        ...
        {
            //Function call is to workaround known bug in Windows 8.1 Preview. Should be removed and called directly at RTM
            GetButtonFromBar("SaveButton").IsEnabled = CanSave;
            GetButtonFromBar("RedEyeButton").IsEnabled = CanFixRedEye;
            GetButtonFromBar("AnnotateButton").IsEnabled = CanAnnotate;
        }
