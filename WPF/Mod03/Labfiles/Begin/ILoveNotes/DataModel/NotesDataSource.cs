...
namespace ILoveNotes.Data
{
    ...
    public sealed class NotesDataSource
    {
        ...
        async private Task LoadJSONFile()
        {
            var types = new List<Type>
                            {typeof (FoodDataItem), 
                             ...
                             typeof (ToDoDataItem), 
                             typeof (NoteBook)};
            var des = new DataContractJsonSerializer(typeof(NoteDataCommon), types);
            var file = await Package.Current.InstalledLocation.GetFileAsync("Data\\Data.JSON");
            try
            {
