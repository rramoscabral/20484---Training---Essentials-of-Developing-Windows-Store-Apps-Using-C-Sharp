...
namespace ILoveNotes.DataModel
{
    public class DataManager
    {
        ...
        private async static Task SaveNotebook(NoteBook nb)
        {
            ...
            DataContractJsonSerializer ser = new DataContractJsonSerializer(typeof(NoteDataCommon), types);
            //Lab 05 : Exercise 1 , Task 2.1 : Declare and open the Notebooks Folder
            var folder = await storage.CreateFolderAsync(FolderNames.Notebooks.ToString(), Windows.Storage.CreationCollisionOption.OpenIfExists);
            try
            {
                using (MemoryStream ms = new MemoryStream())
                {
                    ...
                    //Lab 05 : Exercise 1 , Task 2.2 : Create a FileStorage object and write to it the content 
                    var file = await folder.CreateFileAsync(nb.UniqueId, Windows.Storage.CreationCollisionOption.ReplaceExisting);
                    await FileIO.WriteTextAsync(file, value);
                    await Task.Delay(500);
                }
            }
        ...
        {
            //Lab 05 : Exercise 1 , Task 1.2 : replace StorageFile declerations.
            var storage = Windows.Storage.ApplicationData.Current.LocalFolder;
            ...
            //Lab 05 : Exercise 1 , Task 1.3 : get the file list from the folder
            var files = await folder.GetFilesAsync();
            ...
            foreach (var file in files)
            {
                try
                {
                    //Lab 05 : Exercise 1 , Task 1.4 : open the File for reading using stream object.
                    using (var stream = await file.OpenReadAsync())
                    {
                        var noteB = (NoteDataCommon)des.ReadObject(stream.AsStreamForRead());
                        //Lab 05 : Exercise 1 , Task 1.5 : Add the deserialized note object to the notebook collection
